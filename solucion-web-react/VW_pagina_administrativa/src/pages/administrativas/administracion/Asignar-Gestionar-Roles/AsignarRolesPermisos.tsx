...
const AsignarRolesPermisos: React.FC = () => {
  ...
  const usuariosFiltrados = usuarios.filter((u) => {
    ...
    const esTargetSuperAdmin = susRoles.some(
      (r) => r.rol === ROL_ADMIN_GENERAL,
    );
  ...
  // --- RENDER ---
  if (cargando)
    return <div className="loading-container">Verificando credenciales...</div>;
      ...
      <div className="acceso-denegado-wrapper">
        ...
        <div className={`contenido-roles ${minimizado ? "minimize" : ""}`}>
          ...
          >
            ...
            <p>
              Su usuario no cuenta con los permisos necesarios para acceder a la
              gestión de roles.
            </p>
          </div>
        </div>
          ...
          <table className="tabla-roles">
            ...
            <tbody>
              {usuariosFiltrados.map((usuario) => {
                ...
                const rolesDisponibles = filtrarRolesDisponibles(
                ...
                return [
                  ...rolesUsuario.map((usuarioRol, index) => {
                    ...
                    return (
                      <tr
                        key={`${usuario.id}-${usuarioRol.rol}`}
                        className={esPrimeraFila ? "fila-usuario" : ""}
                      >
                        {esPrimeraFila && (
                          <td
                            rowSpan={
