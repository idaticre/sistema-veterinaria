  ...
  const listarColaboradores = async () => {
    try {
      ...
      const activos = lista.filter(
        (cliente: ColaboradorResponse) => cliente.activo === true,
      );
  ...
  const guardarColaborador = async () => {
    ...
    try {
      if (edicion.id && edicion.id > 0) {
        ...
        await IST.post(`/colaboradores`, edicion);
      }
      listarColaboradores();
      setEdicion(null);
        ...
        <section id="listar-registros">
          ...
          <div className="goated">
            <div className="barra-buscador">
              <input
                type="text"
                placeholder="Ingrese el nombre del colaborador que desea buscar 🔍"
                value={busqueda}
                onChange={(e) => setBusqueda(e.target.value)}
              />
            </div>
            <button
              className="boton-goated anadir-a-goated animacion-goated"
          ...
          <table className="GM-table">
            <thead className="GM-thead">
              <tr className="GM-tr">
                <th className="GM-th" style={{ width: "110px" }}>
                  Código
                </th>
                <th className="GM-th">Nombre</th>
                <th className="GM-th" style={{ width: "100px" }}>
                  Tipo doc.
                </th>
                <th className="GM-th" style={{ width: "110px" }}>
                  Documento
                ...
                <th className="GM-th" style={{ width: "150px" }}>
                  Fecha de ingreso
                </th>
                <th className="GM-th" style={{ width: "200px" }}>
                  Correo
                </th>
              ...
              {filtrado.map((registro) => {
                ...
                return (
                  <tr key={registro.id}>
                    ...
                    <td className="GM-td">
                      {tiposDocumento[Number(registro.idTipoDocumento)] ||
                        "Desconocido"}
                    </td>
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => verMasInformacion(registro)}
                      >
                        Más...
                      </button>
                      <button
      ...
      {mostrarModal && edicion && (
        <div className="ventana-overlay-collab">
          <div className="contenido-ventana-collab">
            <h3>Colaborador</h3>
            <p>
              <strong>Siendo registrado el:</strong> {edicion.fechaIngreso}
            </p>
            <input
              type="text"
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, nombre: nuevoValor.target.value }
                    : null,
                )
              }
            />
            <select
              value={edicion?.sexo ?? "M"}
              onChange={(e) =>
                setEdicion(
                  edicion
                    ? { ...edicion, sexo: e.target.value as "M" | "F" }
                    : null,
                )
              }
            >
            ...
            >
              {tiposPersonasJuridicas.map((tipoPersonaJuridica) => (
                <option
                  key={tipoPersonaJuridica.id}
                  value={tipoPersonaJuridica.id}
                >
                  {tipoPersonaJuridica.nombre}
                </option>
              ))}
            </select>
            <select
              value={edicion.idTipoDocumento}
              onChange={(nuevoValor) =>
                setEdicion((prev) =>
                  prev
                    ? {
                        ...prev,
                        idTipoDocumento: Number(nuevoValor.target.value),
                      }
                    : null,
                )
              }
            ...
            <input
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, documento: nuevoValor.target.value }
                    : null,
                )
              }
            />
            <input
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, telefono: nuevoValor.target.value }
                    : null,
                )
              }
            />
            <input
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, correo: nuevoValor.target.value }
                    : null,
                )
              }
            />
            <input
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, direccion: nuevoValor.target.value }
                    : null,
                )
              }
            />
            <input
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, ciudad: nuevoValor.target.value }
                    : null,
                )
              }
            />
            <input
              ...
              onChange={(nuevoValor) =>
                setEdicion(
                  edicion
                    ? { ...edicion, distrito: nuevoValor.target.value }
                    : null,
                )
              }
            />
      ...
      {mostrarModalInformativo && masInformacion && (
        <div className="ventana-overlay">
          <div className="contenido-ventana-collab">
            ...
            <div className="info-extensiva">
              <strong>Sexo: </strong>
              {masInformacion.sexo === "M" ? "Masculino" : "Femenino"}
            </div>
            <div className="info-extensiva">
              <strong>Ciudad: </strong>
              {masInformacion.ciudad}
            </div>
            <div className="info-extensiva">
              <strong>Distrito: </strong>
              {masInformacion.distrito}
            </div>
          </div>
