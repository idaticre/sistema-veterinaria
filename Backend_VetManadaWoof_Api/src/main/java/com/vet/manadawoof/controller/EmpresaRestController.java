...
public class EmpresaRestController {
    ...
    public ResponseEntity<List<EmpresaEntity>> listarEmpresas() {
        List<EmpresaEntity> empresas = service.listarEmpresas();
        if (empresas.isEmpty()) {
    ...
    public ResponseEntity<String> actualizarEmpresa(
        ...
        EmpresaEntity existente = service.obtenerEmpresa(id);
        if (existente == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("La empresa con id " + id + " no existe");
        }

        empresa.setId(id);
        service.actualizarEmpresa(empresa);

        return ResponseEntity.status(HttpStatus.OK)
                .body("Empresa actualizada correctamente");
    }
}
